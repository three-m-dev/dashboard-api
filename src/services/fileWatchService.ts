...
export class FileWatchService {
	...
	private getCurrentData(): Promise<DataRow[]> {
		return new Promise((resolve, reject) => {
			fs.access(this.currentDataPath, fs.constants.F_OK, (err) => {
				if (err) {
					return reject(new Error(`File not found: ${this.currentDataPath}`));
				}
	...
	private getNewData(): DataRow[] {
		if (!fs.existsSync(this.watchedFilePath)) {
			console.error(`File not found: ${this.watchedFilePath}`);
			return [];
		}
	...
	public async watch(): Promise<void> {
		if (!fs.existsSync(this.watchedFilePath)) {
			const fileName = path.basename(this.watchedFilePath);
