...
export class ProductionService {
  ...
  updateProductionLog = async (logId: string, updates: Partial<IProductionLog>) => {
    const log = await db.ProductionLog.findByPk(logId);
    if (!log) {
      throw new Error(`ProductionLog with ID ${logId} not found.`);
    }

    Object.entries(updates).forEach(([key, value]) => {
      if (key !== 'id' && key !== 'company' && key !== 'weekOf') {
  ...
  public deleteProductionLog = async (logId: string) => {
    const deletionResult = await db.ProductionLog.destroy({
      where: {
        id: logId,
      },
    });
    ...
    return { message: `ProductionLog with ID ${logId} successfully deleted.` };
  };
}
